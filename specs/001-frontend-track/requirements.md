...
> Added `OnboardingCheck` client component to `app/page.tsx` that inspects `localStorage.getItem('cadence_onboarding_completed')` on initial visit and automatically routes first-time visitors to `/onboarding`. Ensured "How it works" link is responsive across desktop and mobile viewports.
...
> **Phase 7 Spec Sync (2026-09-17):**
> Updated `FlashCard.tsx` to handle standard `{{blank}}` tokens generated by Claude alongside legacy `{{c1::...}}` syntax, rendering `[ ... ]` pills on the front and contextual sentence highlights on the flipped answer side. Fixed `CardStack.tsx` pointer capture so swiping left/right to rate works continuously when the card is flipped.
